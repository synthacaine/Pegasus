...
interface IRegistration {
    function checkUserExists(address _addr) external view returns(bool);
    function checkIfExpired(address _addr) external view returns(bool);
...
contract ArbitrageMain is Ownable {
    ...
    address public providerAddress = 0x5E52dEc931FFb32f609681B8438A51c675cc232d;

    mapping(address => uint) public totalUserSimpleArbitrages;
    mapping(address => uint) public totalUserSimpleArFlashLoanbitrages;
    mapping(address => uint) public totalUserTriangularArbitrages;
    mapping(address => uint) public totalUserTriangularFlashLoanArbitrages;

    mapping(address => address[]) public userSimpleArbitrages;
    mapping(address => address[]) public userSimpleFlashLoanArbitrages;
    mapping(address => address[]) public userTriangularArbitrages;
    mapping(address => address[]) public userTriangularFlashLoanArbitrages;
    ...
    constructor(address _regAddress) public {
        registration = IRegistration(_regAddress);
        // simpleArbitrage = ISimpleArbitrage(_simpleArbitrage);
        // triangularArbitrage = ITriangularArbitrage(_triangularArbitrage);
    }

    modifier isRegistered(address _user) {
      require(!registration.checkIfExpired(msg.sender) || !registration.checkIfExpired(_user), "Registration expired");
      _;
    }
    ...
    function modifyRegistrationAddress(address _registrationAddress) external onlyOwner {
        registration = IRegistration(_registrationAddress);
    }

    function callSimpleArbitrage(address _user, address _exchangeA, address _exchangeB, address _tokenA, address _tokenB, uint256 _amountIn) public payable isRegistered(_user) {
        ...
        userSimpleArbitrages[_user].push(contractA);
        ...
        totalUserSimpleArbitrages[_user]++;

        emit SimpleArbitrageDeployed(_user, contractA);
    }  

    function callSimpleFlashLoan(address _user, address _exchangeA, address _exchangeB, address _tokenA, address _tokenB, uint256 _amountIn) public payable isRegistered(_user) {
        address payable contractA = address(new FlashLoanSimpleArbitrage(providerAddress, _exchangeA, _exchangeB, _tokenA, _tokenB, _user));
        userSimpleFlashLoanArbitrages[_user].push(contractA);
        ...
        totalUserSimpleArFlashLoanbitrages[_user]++;

        emit SimpleFlashLoanArbitrageDeployed(_user, contractA);
    }

    function callTriangularArbitrage(address _user, address _exchangeA, address _exchangeB, address _tokenA, address _tokenB, address _tokenC, uint256 _amountIn) public payable isRegistered(_user) {
        ...
        userTriangularArbitrages[_user].push(contractA);
        ...
        totalUserTriangularArbitrages[_user]++; 


        emit TriangularArbitrageDeployed(_user, contractA);
    }

    function callTriangularFlashLoan(address _user, address _exchangeA, address _exchangeB, address _tokenA, address _tokenB, address _tokenC, uint256 _amountIn) public payable isRegistered(_user) {
        address payable contractA = address(new FlashLoanTriangularArbitrage(providerAddress, _exchangeA, _exchangeB, _tokenA, _tokenB, _tokenC, _user));
        userTriangularFlashLoanArbitrages[_user].push(contractA);
        ...
        totalUserTriangularFlashLoanArbitrages[_user]++;
